const convertBtn = document.querySelector('.convert-button') // convert button
const URLinput = document.querySelector('.url-input') // url input
const convertTxt = document.getElementById('converting') // text informing user of conversion process
const embedDiv = document.getElementById('embedDiv') // div element to store convert text and bar
const progress = document.getElementById('myProgress') // progress bar
const url = document.getElementById('URL').value // url value from python parameters
...
if (
  String(url).includes('mp3') &&
  !String(window.location.href).includes('download')
) {
  const link = document.getElementById('download') // a tag for automatic download
  ...
  window.location.href = `${window.location.href.substring(
    0,
    22
  )}download${window.location.href.substring(29)}` // changes url to download url
}
...
let i = 0 // variable for width of inner progress bar
// move progress bar
function move () {
  if (i === 0) {
    i = 1
